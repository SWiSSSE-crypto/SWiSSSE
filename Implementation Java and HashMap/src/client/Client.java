...
public class Client {
	private static final int DOCUMENT_SIZE = 2048;
	private static final int Document_Expansion = 2;
	...
	/* Class constructor
	 */
	public Client(List<Document> documents, int grouping) throws GeneralSecurityException
	{
		// Key generation
		this.key = Crypto.key_gen();
		this.signingKey = new SecretKeySpec(key.getEncoded(), "HmacSHA1");
		...
		for (int ii = 0; ii < Math.ceil(keywords.size() / (double) this.grouping); ii++)
		{
			int count_max = keyword_count.get(keywords.get(ii*this.grouping));
			for (int jj = ii*this.grouping; (jj < (ii+1) * this.grouping) & (jj < keywords.size()); jj++)
				count_max = (keyword_count.get(keywords.get(jj)) > count_max) ? (keyword_count.get(keywords.get(jj))): count_max;
				
			for (int jj = ii*this.grouping; (jj < (ii+1) * this.grouping) & (jj < keywords.size()); jj++)
				keyword_max.put(keywords.get(jj), count_max * 2);
		}
		
		this.array_counter = new int[documents.size()*Document_Expansion];
		Arrays.fill(this.array_counter, 0);
		document_identifier_counter = documents.size()*Document_Expansion;
	}
	
	
	/* Pad the database with fake documents
	 */
	...
	/* setup database
	 */
	public void setup(List<Document> documents, HashMap<String, byte[]> EDB, Timer timer_setup) throws GeneralSecurityException
	{	
		timer_setup.start();
		...
		for (int array_loc = 0; array_loc < documents.size(); array_loc++)
		{
			...
			String doc_address = Crypto.SHA1(this.signingKey, Integer.toString(array_loc) + '-' + Integer.toString(array_counter[array_loc]));
	...
	/* setup database
	 */
	public void setup_fake(List<Document> documents, HashMap<String, byte[]> EDB, Timer timer_setup) throws GeneralSecurityException
	{	
		for (Document document: documents)
			document.setReal(false);
		...
		for (int array_loc = 0; array_loc < documents.size(); array_loc++)
		{
			...
			byte[] document_data_enc = Crypto.GCM_encrypt(this.signingKey, random, document_data);
			EDB.put(doc_address, document_data_enc);
	 ...
	 */
	public void single_keyword_query(String keyword, SecureRandom random, HashMap<String, byte[]> EDB, List<Document> documents, Timer timer_query, Timer timer_write_back) throws GeneralSecurityException, IOException
	{
		timer_query.start();
		Hashtable<String, String> keyword_to_document_address = new Hashtable<String, String>();
		get_document_addresses(keyword, EDB, keyword_to_document_address);
		
		Hashtable<String, Document> keyword_to_document = new Hashtable<String, Document>();
		get_documents_from_addresses(EDB, keyword_to_document_address, keyword_to_document);
		timer_query.stop();
	...
	/* Single keyword query
	 */
	public Integer insertion_query(Document document, SecureRandom random, HashMap<String, byte[]> EDB, List<Document> documents, Timer timer_query) throws GeneralSecurityException, IOException
	{
	 ...
	 */
	private void get_document_addresses(String keyword, HashMap<String, byte[]> EDB, Hashtable<String, String> keyword_to_document_address) throws GeneralSecurityException
	{	
		List<String> lookup_addresses_raw = new ArrayList<String>();
		List<String> lookup_addresses = new ArrayList<String>();
		...
		for (String address_raw: lookup_addresses_raw)
			lookup_addresses.add(Crypto.SHA1(this.signingKey, address_raw));
		...
		// Make query
		List<byte[]> document_addresses_enc = new ArrayList<byte[]>(); //.mget(lookup_addresses.toArray(new String[0]));
		for (String lookup_address: lookup_addresses)
			document_addresses_enc.add(EDB.remove(lookup_address));
	 ...
	 */
	private void get_documents_from_addresses(HashMap<String,byte[]> EDB, Hashtable<String, String> keyword_to_document_address, Hashtable<String, Document> keyword_to_document) throws GeneralSecurityException, IOException
	{
		int void_counter = 0;
		int next_address = 0;
		...
		List<String> document_addresses = new ArrayList<String>();
		for (String keyword: keywords_list)
		{
			...
			if (Integer.parseInt(keyword_to_document_address.get(keyword)) < this.array_counter.length)
			{
				...
				String document_address = Crypto.SHA1(this.signingKey, address);
				document_addresses.add(document_address);
		...
		List<byte[]> document_enc = new ArrayList<byte[]>(); 
		for (String document_address: document_addresses)
			document_enc.add(EDB.remove(document_address));
		...
		for (String keyword: keywords_list)
		{
			...
			if (Integer.parseInt(keyword_to_document_address.get(keyword)) < this.array_counter.length)
			{
				String document_dec = Crypto.GCM_decryption(key, document_enc.get(counter));
	 ...
	 */
	private void write_back(HashMap<String, byte[]> EDB, SecureRandom random) throws GeneralSecurityException, IOException
	{		
		...
		int write_back_counter = document_stash.size() / 2;
		...
		{			
			...
			String document_address = Crypto.SHA1(this.signingKey, Integer.toString(array_loc) + '-' + Integer.toString(this.array_counter[array_loc]));
			byte[] document_enc = Crypto.GCM_encrypt(key, random, document.get_string(DOCUMENT_SIZE));
			EDB.put(document_address, document_enc);
		...
		for (int ii = 0; ii < lookup_stash_keys.size() * 0.5; ii++)
		{
			...
			String lookup_address = Crypto.SHA1(this.signingKey, lookup_stash_keys.get(ii) + "-" + index.get_time_stamp());
