...
public class Client {
	...
	/* Class constructor
	 */
	public Client(List<Document> documents, String grouping_mode) throws GeneralSecurityException
	{
		// Key generation
		...
		for (int ii = 0; ii < Math.ceil(keywords.size() / 50.0); ii++)
		{
			int count_max = keyword_count.get(keywords.get(ii*50));
			for (int jj = ii*50; (jj < (ii+1) * 50) & (jj < keywords.size()); jj++)
	...
	/* setup database
	 */
	public void setup(List<Document> documents, DataInputStream input_stream, DataOutputStream output_stream, Timer timer_setup) throws GeneralSecurityException, UnknownHostException, IOException
	{
		...
		for (int array_loc = 0; array_loc < documents.size(); array_loc++)
		{
			...
			byte[] doc_address = Crypto.SHA1(this.signingKey, Integer.toString(array_loc) + '-' + Integer.toString(array_counter[array_loc]));
			...
			{
				byte[] lookup_address = Crypto.SHA1(this.signingKey, keyword + "-0"); 
	 ...
	 */
	public void single_keyword_query(String keyword, SecureRandom random, Timer timer_query, Timer timer_write_back) throws GeneralSecurityException, IOException
	{
		timer_query.start();
	...
	/* Single keyword query
	 */
	public Integer insertion_query(Document document, SecureRandom random, Timer timer_query) throws GeneralSecurityException, IOException
	{
	...
	{	
		...
		{
			byte[] address = Crypto.SHA1(this.signingKey, address_raw);
	...
	private void get_documents_from_addresses(Hashtable<String, String> keyword_to_document_address, Hashtable<String, Document> keyword_to_document, Timer timer_query) throws GeneralSecurityException, IOException
	{
		...
		for (String keyword: keywords_list)
		{
			...
			if (Integer.parseInt(keyword_to_document_address.get(keyword)) < this.array_counter.length)
			{
				...
				byte[] document_address = Crypto.SHA1(this.signingKey, address);
	...
	{		
		...
		{			
			...
			byte[] document_address = Crypto.SHA1(this.signingKey, Integer.toString(array_loc) + '-' + Integer.toString(this.array_counter[array_loc]));
		...
		for (int ii = 0; ii < lookup_stash_keys.size() * 0.5; ii++)
		{
			...
			byte[] lookup_address = Crypto.SHA1(this.signingKey, lookup_stash_keys.get(ii) + "-" + index.get_time_stamp());
